...
PyGitHub + GitHub GraphQL · DeepSeek（Flash 检索 / **Pro 调度反思**）· Streamlit
...
> D4+ 核心 agent 逻辑（Supervisor/Planner/Reflect）用 Pro，检索 Worker 用 Flash。

## 约定
- 密钥在 `.env`（已 gitignore），**绝不提交**
- 原始数据 `data/raw/`，Qdrant `data/qdrant/`，SQLite `data/tdas.db`（均在 gitignore）
- 核心 agent 逻辑（Supervisor/Planner/Reflect）用 **Pro** 模型，检索 Worker 用 **Flash**
